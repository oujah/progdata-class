...
Follows the format: select(gapminder, arguments).
...
Selecting multiple variables may be tedious. We could do this easily
```{r}
# This will work:
...
rename(gapminder, Continent = continent,  Year = year, Country = country)
```
...
```{r}
gapminder |>filter(pop >100000000) |> filter(country=="Brazil"|country=="China")
...
```{r}
gapminder |> mutate(
  lifeExp = if_else(country=="Canada" & year ==1952, 
                    true = 70,
                    false = lifeExp))
```
